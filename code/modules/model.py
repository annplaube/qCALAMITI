...
class qCALAMITI:
    ...
    def load_dataset(self, dataset_dirs, data_names, orientations, batch_size):
        ...
        self.num_mod = len(train_dataset.imgs)
    ...
    def cal_loss(self, betas, thetas, dataset_id, rec_imgs=None, imgs=None, img_ids=None,
        ...
        if rec_imgs:
            ...
            curr_batch_size = betas[0].shape[0] * self.num_mod
    ...
    def save_image(self, imgs, file_prefix):
        ...
        for ch in range(num_channels):
            ...
            grid = utils.make_grid(tensor=img_save, nrow=4, normalize=False, value_range=(0,1))
            if num_channels > 1:
    ...
    def encode_single_vol(self, vol, img_slc, out_dir, prefix, orientation="axial", img_hdr=None, img_affine=None, theta_only=False):
        """
        Encode image volume into beta and theta.
        Assumes input vol and img_slc are axial. Orientation is encoding direction, volumes saved in original input orientation.
        Image volumes should come in axial orientation.
        if theta_only == True, do not encode and save beta.
        """
        mkdir_p(out_dir)
        if not isinstance(img_slc, list):
            img_slc = [img_slc,]
        with torch.set_grad_enabled(False):
            self.da_beta.eval()
            self.da_theta.eval()
            ...
            if not theta_only:
                ...
                for slc in range(vol.shape[0]):
                    img = (vol[[slc],...]).to(self.device).unsqueeze(1) # generate 4d tensor
                ...
                # save beta image
                if orientation == 'axial':
                    img_save = np.array(beta.cpu().squeeze().permute(0,2,3,1))
                elif orientation == 'coronal':
                    img_save = np.array(beta.cpu().squeeze().permute(2,3,0,1))
                elif orientation == 'sagittal':
                    img_save = np.array(beta.cpu().squeeze().permute(2,0,3,1))
                img_save = nib.Nifti1Image(img_save, img_affine, img_hdr)
                file_name = os.path.join(out_dir, f'{prefix}_beta_{orientation}.nii')
                nib.save(img_save, file_name)

                # save original image (only do once)
                if orientation == 'axial':
                    img_save = nib.Nifti1Image(vol, img_affine, img_hdr)
                    file_name = os.path.join(out_dir, f'{prefix}_ori.nii')
                    nib.save(img_save, file_name)

            # calculate theta -- same for all directions, only do for axial
            if orientation == "axial":
                mus = []
                for slc in img_slc:
    ...
    def decode_single_vol(self, beta, theta, out_dir, prefix, orientation="axial", img_hdr=None, img_affine=None):
        """Decode beta volume (pass in axial orientation).
        orientation parameter states the desired decoding orientation (should match encoding direction).
        Saves decoded image volume in original orientation.
        """
        mkdir_p(out_dir)
        if orientation == "axial":
            beta = beta
        ...
        with torch.set_grad_enabled(False):
            ...
            betas = [beta[[i],...] for i in range(batch_size)]
            rec_imgs = []
            for b in betas:
                b = b.to(self.device)
                combined = torch.cat([b, theta.repeat(b.size()[0], 1, b.size()[2], b.size()[3])], dim=1)
                rec_imgs.append(self.decoder(combined).cpu())
            rec_img = torch.cat(rec_imgs, dim=0)

            if orientation == 'axial':
                img_save = np.array(rec_img.cpu().permute(0,2,3,1).squeeze())
            elif orientation == 'coronal':
                img_save = np.array(rec_img.cpu().permute(2,3,0,1).squeeze())
            elif orientation == 'sagittal':
                img_save = np.array(rec_img.cpu().permute(2,0,3,1).squeeze())
            img_save = nib.Nifti1Image(img_save, img_affine, img_hdr)
            file_name = os.path.join(out_dir, f'{prefix}_recon_{orientation}.nii')
            nib.save(img_save, file_name)
